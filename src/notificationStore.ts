...
export async function saveNotifications(items: AppNotification[]) {
  ...
  const { error } = await supabase.from(NOTIFICATION_TABLE).upsert(items.map(notificationToRow), { onConflict: 'id' });
  if (error) {
    console.warn('Supabase notification save failed. Local fallback is still updated.', error);
  }
}
